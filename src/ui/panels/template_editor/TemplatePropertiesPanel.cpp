...
void TemplatePropertiesPanel::Render() noexcept
{
    ...
    // Branch parameters (physics-centric)
    ...
    ImGui::SliderFloat("Fade start", &fireworkTemplate->branchTemplate.fadeStartRatio, 0.0f, 1.0f, "%.2f");
    ...
    // Visual mode
    ImGui::Text("Famille visuelle");
    {
        int mode = static_cast<int>(fireworkTemplate->branchTemplate.visualMode);
        const char* modes[] = { "Comet", "Sparkle" };
        if (ImGui::Combo("Visual Mode", &mode, modes, IM_ARRAYSIZE(modes))) {
            if (mode < 0) mode = 0;
            if (mode > 1) mode = 1;
            fireworkTemplate->branchTemplate.visualMode = static_cast<BranchDescriptor::VisualMode>(mode);
        }
    ...
    // Trails
    ImGui::Text("Traînée (ruban)");
    ImGui::Checkbox("Trail enabled", &fireworkTemplate->branchTemplate.trailEnabled);
    ImGui::DragFloat("Trail width", &fireworkTemplate->branchTemplate.trailWidth, 0.001f, 0.0f, 1.0f, "%.3f");
    if (fireworkTemplate->branchTemplate.trailWidth < 0.0f) fireworkTemplate->branchTemplate.trailWidth = 0.0f;
    ImGui::DragFloat("Trail duration (s)", &fireworkTemplate->branchTemplate.trailDuration, 0.01f, 0.0f, 5.0f, "%.2f");
    ...
    if (ImGui::CollapsingHeader("Extensions (WIP)")) {
    ...
    // Testability helpers: derived metrics for the chosen model (linear drag)
